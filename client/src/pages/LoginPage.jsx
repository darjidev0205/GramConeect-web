...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    // MOCK API CALL
    const mockUser = { id: 1, name: "Test User", email, role: email.includes("admin") ? 'admin' : email.includes("agent") ? 'agent' : 'user' };
        ...
        <Card className="w-full max-w-md shadow-2xl border-border/50 bg-card/60 backdrop-blur-xl">
          ...
          <CardContent>
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <Button type="submit" className="w-full h-12 text-base rounded-lg mt-2">Sign In</Button>
            </form>
          </CardContent>

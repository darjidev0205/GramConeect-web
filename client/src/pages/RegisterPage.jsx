      ...
      >
        ...
        <Card className="w-full shadow-2xl border border-white/10 bg-[#0d1428]/95 backdrop-blur-2xl text-white rounded-3xl p-2 sm:p-4">
          <CardHeader className="space-y-1 text-center pb-6">
            <CardTitle className="text-2xl font-extrabold tracking-tight font-display text-white">Create Account</CardTitle>
            <CardDescription className="text-xs text-slate-400">Join GramConnect network and start delivering opportunities</CardDescription>
          </CardHeader>
          <CardContent>
            {error && (
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-mono uppercase font-bold text-slate-300 block mb-1">FULL NAME</label>
                <Input 
                  type="text" placeholder="Suresh Kumar" 
              ...
              <div className="flex items-center gap-2 text-xs text-slate-300 pt-1 pb-2">
                <input 
                  type="checkbox" 
                  id="terms" 
                  checked={termsAccepted} 
                  onChange={e => setTermsAccepted(e.target.checked)}
                  className="rounded border-white/20 bg-white/5 text-cyan-400 focus:ring-0 cursor-pointer w-4 h-4"
                  required
                />
                <label htmlFor="terms" className="cursor-pointer select-none">I accept the GramConnect Terms & Partner Policy</label>
              </div>
          ...
          <CardFooter className="flex flex-col items-center gap-4 text-xs text-slate-400 pt-4 border-t border-white/10 mt-4">
            <div>
              Already have an account? <Link to="/login" className="ml-1 text-cyan-400 hover:underline font-bold">Sign in</Link>
            </div>
